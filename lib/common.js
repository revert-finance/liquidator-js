...
// gas price for tx cost estimation
async function getGasPriceCached(force) {
    const now = new Date().getTime()
    if (force || !gasPriceCache.lastUpdated || gasPriceCache.lastUpdated + 10000 < now) {
        const price = await provider.getGasPrice()
        gasPriceCache.price = price
...
async function checkAndIncreasePoolHistory(position, gasPrice) {
    if (!observationCardinalityIncreases[position.poolAddress]) {
        observationCardinalityIncreases[position.poolAddress] = true
        const minCardinatily = getObservationCardinalityNextMin()

        if (minCardinatily > 0) {
            
            // on mainnet skip if fee not enough
            if (network === "mainnet") {
                if (position.feeValueEth.lt(gasPrice.mul(140094))) {
                    return
                }
            }

            const poolContract = new ethers.Contract(position.poolAddress, POOL_ABI, provider)
            const slot0 = await poolContract.slot0()
            if (slot0.observationCardinalityNext < minCardinatily) {
                const tx = await poolContract.populateTransaction.increaseObservationCardinalityNext(minCardinatily)
                const { hash, error } = await executeTx(tx, async(success) => {
                })
    
                if (hash) {
                    const text = `Increasing Observation Cardinality for ${getRevertUrlForDiscord(position.tokenId)} - ${getExplorerUrlForDiscord(hash)}`
                    console.log(text)
                    await sendDiscordInfo(text)
                } else {
...
async function quoteUniversalRouter(tokenIn, tokenOut, tokenInDecimals, tokenOutDecimals, amountIn, recipient, slippageX10000, deadline, feeX10000, feeRecipient) {
    ...
    for (const routePart of route.route) {
        ...
        pools.push(...routePart.poolAddresses)
...
// estimated gas limits per network / operation to decide if execute or not
function getGasLimitForOperation(type) {
    return {
        "mainnet": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251)),
        "polygon": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251)),
        "optimism": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251)),
        "arbitrum": (type === "auto-range" ? 1348780 : (type === "auto-exit-swap" ? 1575043 : 957004)),
        "bnb": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251)),
        "evmos": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251)),
        "base": (type === "auto-range" ? 1037140 : (type === "auto-exit-swap" ? 640853 : 364251))
    }[network];
}
